...
export default {
  ...
  methods: {
    async getAvailable() {
      ...
      await Axios.get(`/dashboard/?email=${email}`)
        .then((response) => {
      ...
      await Axios.get(`/services/?email=${email}`)
        .then((response) => {
    ...
    filterServices(data) {
      for (const entry in this.availableServices) {
        if (Object.hasOwnProperty.call(this.availableServices, entry)) {
          for (let widget = this.availableServices[entry].widgets.length; widget-- ;) {
            if (this.doesRequireOauth(data, this.availableServices[entry].widgets[widget])) {
