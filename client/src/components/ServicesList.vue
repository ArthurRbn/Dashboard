  ...
  methods: {
    async isConnected() {
      if (this.email.length < 1)
        return;
      await Axios.get(`/services/?email=${this.email}`)
        .then((response) => {
    ...
    async revokeToken() {
      Axios.post(`/reddit/revoke/`, {email: this.email})
        .then((response) => {
    ...
    async revokeTokenTwitch() {
      Axios.post(`/twitch/revoke/`, {email: this.email})
          .then((response) => {
            console.log("revoke operation", response.data);
    ...
    redirectOAuth() {
      ...
      location.assign(`https://www.reddit.com/api/v1/authorize?api_key&client_id=Y3wx2SfTchkt7Ai2v2CGdg&redirect_uri=${process.env.VUE_APP_REDDIT_OAUTH_CALLBACK}&response_type=code&scope=identity%20account&state=my_random_string&duration=permanent`);
    },
    redirectTwitchOAuth() {
      ...
      location.assign(`https://id.twitch.tv/oauth2/authorize?client_id=84ywaahm7orgmuraq6aqqnsb8a9934&redirect_uri=${process.env.VUE_APP_TWITCH_OAUTH_CALLBACK}&response_type=code&scope=user:read:email&state=`);
    }
  },
  mounted() {
