...
exports.getTwitchAccess = async (req, res) => {
    if (clientId.search("none") != -1 || clientSecret.search("none") != -1) {
        console.log('ERROR: Missing env variables, operation cancelled');
        throw new Error({ err: "Missing env variables, operation cancelled" });
    }
    try {
        if (process.env.TWITCH_OAUTH_CALLBACK.search("none") != -1) {
            console.log('ERROR: Missing env variables, operation cancelled');
            throw new Error({ err: "Missing env variables, operation cancelled"});
        }
        let result = await axios.post(`https://id.twitch.tv/oauth2/token?client_id=${clientId}&client_secret=${clientSecret}&code=${req.query["code"]}&grant_type=authorization_code&redirect_uri=${process.env.TWITCH_OAUTH_CALLBACK}`);
...
async function addService(service_name, base_url, data, email) {
    ...
    // If service already exists revoke old token and replace by new
